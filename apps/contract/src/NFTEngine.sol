...
contract NFTEngine {
        ...
        returns (string memory startColor, string memory endColor)
    {
        ...
        // relativePriceChange is the relative price * 1e8
        // now if we want to convert this to precentages
        // we can assume relativePriceChange is 1e6.
        if (isPositive) {
            startColor = absRelativePrice < 1e5
                ? "rgba(51,195,26,0)" // <0.1%
                : absRelativePrice < 5e5
                    ? "rgba(51,195,26,0.1)" // 0.1-0.5%
                    : absRelativePrice < 1e6
                        ? "rgba(51,195,26,0.2)" // 0.5-1%
                        : absRelativePrice < 3e6
                            ? "rgba(51,195,26,0.3)" // 1% - 3%
                            : absRelativePrice < 5e6
                                ? "rgba(51,195,26,0.5)" // 3% - 5%
                                : absRelativePrice < 8e6
                                    ? "rgba(51,195,26,0.7)" // 5% - 8%
                                    : absRelativePrice < 10e6
                                        ? "rgba(51,195,26,0.9)" // 8-10%
                                        : "rgba(51,195,26,1)"; // < 10%
            endColor = "#33c31a";
        } else {
            startColor = "#c31a1a";
            endColor = absRelativePrice < 1e5
    ...
    function generateWithMeta(Meta memory meta, uint256 tokenId) external view returns (string memory) {
        ...
        string memory part5 = string.concat(
            '<g id="Layer_x0020_1" filter="url(#glow-planet)">',
        ...
        return string(abi.encodePacked(part1, part2, part3, part5, part6));
    }
}
